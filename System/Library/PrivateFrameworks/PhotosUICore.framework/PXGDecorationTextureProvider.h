/*
                       * This header is generated by classdump-dyld 1.5
                       * on Wednesday, October 27, 2021 at 3:16:13 PM Mountain Standard Time
                       * Operating System: Version 13.5.1 (Build 17F80)
                       * Image Source: /System/Library/PrivateFrameworks/PhotosUICore.framework/PhotosUICore
                       * classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos. Updated by Kevin Bradley.
                       */

#import <PhotosUICore/PhotosUICore-Structs.h>
#import <PhotosUICore/PXGCGImageTextureProvider.h>

@interface PXGDecorationTextureProvider : PXGCGImageTextureProvider
-(NSRange)requestTexturesForSpritesInRange:(PXGSpriteIndexRange)arg1 geometries:(SCD_Struct_PX13*)arg2 styles:(SCD_Struct_PX16*)arg3 infos:(SCD_Struct_PX17*)arg4 inLayout:(id)arg5 ;
-(void)_requestTextureForDecorationInfo:(PXGDecorationSpriteInfo)arg1 debugDecoration:(id)arg2 targetSize:(CGSize)arg3 cornerRadius:(SCD_Struct_PX15)arg4 screenScale:(double)arg5 requestID:(int)arg6 viewEnvironment:(id)arg7 userInterfaceDirection:(unsigned long long)arg8 ;
@end

