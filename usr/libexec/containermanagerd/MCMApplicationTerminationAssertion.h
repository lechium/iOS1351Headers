//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Sep 26 2020 13:48:20).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

@class FBSApplicationTerminationAssertion;

@interface MCMApplicationTerminationAssertion : NSObject
{
    FBSApplicationTerminationAssertion *_termAssertion;	// 8 = 0x8
}

- (void).cxx_destruct;	// IMP=0x000000010004e1d4
@property(retain, nonatomic) FBSApplicationTerminationAssertion *termAssertion; // @synthesize termAssertion=_termAssertion;
- (void)dealloc;	// IMP=0x000000010004e144
- (void)invalidate;	// IMP=0x000000010004e0b0
- (id)initWithBundleIdentifier:(id)arg1 reason:(unsigned char)arg2;	// IMP=0x000000010004dd5c

@end

