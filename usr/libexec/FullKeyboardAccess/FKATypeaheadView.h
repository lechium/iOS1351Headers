//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Sep 26 2020 13:48:20).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import "FKAPlatterContainerView.h"

@class FKATypeaheadTextField, UITextField;

@interface FKATypeaheadView : FKAPlatterContainerView
{
    FKATypeaheadTextField *_textField;	// 8 = 0x8
}

- (void).cxx_destruct;	// IMP=0x0000000100013f7c
@property(retain, nonatomic) FKATypeaheadTextField *textField; // @synthesize textField=_textField;
- (double)platterWidth;	// IMP=0x0000000100013f50
- (id)initWithFrame:(struct CGRect)arg1;	// IMP=0x0000000100013784
@property(readonly, nonatomic) UITextField *typeaheadTextField;

@end

